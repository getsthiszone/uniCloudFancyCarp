<template>
	<view>
		...
		<block v-else>
			<view>
				<swiper class="screen-swiper" :indicator-dots="true" :circular="true" :autoplay="false" interval="5000" duration="500"
				 indicator-active-color="#FFF" style="height: 750upx;">
		...
		<view class="foot cu-bar bg-white tabbar border shop solids-top  " style="z-index: 9;">
			...
			<view class="btn-group">
				<button class="cu-btn bg-blue round shadow-blur" style="width: 420upx;" @tap="openPoup('sizepoup')">立即购买</button>
			</view>
		</view>
		...
		<uniPopup ref="sizepoup" type="bottom">
			<view class="cu-dialog bg-white" style="width: 100%;z-index: 10;border-radius: 0rpx;">
				<view class="cu-bar bg-white" style="border-bottom: solid 1rpx #f3f3f3;">
					<view class="flex " style="padding:50upx 30upx;">
						<Gimage :src="goods.images" mode="aspectFill" style="width: 180rpx;height: 180rpx;border-radius: 10rpx; margin-right: 20rpx; "></Gimage>
						<view class=" " style="width: 400upx;">
							<view class="text-left text-cut2" style="line-height: 48upx;font-size: 30upx; height: 100upx;">{{ goods.title }}</view>
							<view class="text-left price-color" style=" width: 100%;font-size: 32upx; margin-top: 20upx;">
								￥{{ goods.price ? goods.price : goods.price }}
							</view>
						</view>
					</view>
				</view>
				<scroll-view scroll-y="true" style="height: 350upx;width: 100%;">
					<view style="height: 40rpx;"></view>
					<view class="padding-li text-left bg-white" style="padding: 0upx 40upx;">
						<view class="text-left" style="height: 60upx;">时间</view>
						<view class="no-h-view" style="padding-bottom: 30upx;">
							<view class="x_view" :class="attrCur==index1 ? 'attr-action' : ''" v-for="(item1, index1) in time_array" :key="index1"
							 @tap="getattr" :data-index="index1">
								{{ item1.start }}-{{ item1.end }}
							</view>
						</view>
					</view>
				</scroll-view>
				<view class="buydbtn" @tap="buy">确定</view>
			</view>
		</uniPopup>
		...
		methods: {
			...
			getinfo() {
				that.$request.request({
					url: that.api.appointment.detail,
					success(res) {
						that.goods = res;
						that.Skeleton = false;
						that.time_array = res.time_array
					}
				});
			},
			buy() {
				...
				that.closePoup("sizepoup");
				let data = {
					goods: that.goods,
					attrCur: that.attrCur
				}
				uni.setStorageSync('ordersite', data);
				uni.navigateTo({
					url: '/pages/shop/siteCorder/siteCorder'
				});
			},
			getattr(e) {
				that.attrCur = e.currentTarget.dataset.index;
			}
		},
		onLoad(option) {
			that = this;
