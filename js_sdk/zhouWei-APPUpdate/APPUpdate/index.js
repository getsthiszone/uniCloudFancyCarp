...
export const getServerNo = function(version, isPrompt = false, callback) {
	 ...
	 */
	require.request({
		url: api.default.version,
		success(res) {
			let detail = res.detail;
			console.log("获取版本信息", detail)
			let data = {}
			if (platform == "android") {
				data = {
					versionCode: detail.newversion,
					versionName: '更新',
					versionInfo: detail.content,
					downloadUrl: detail.downloadurl,
					forceUpdate: detail.enforce
				}
				let text = `当前版本:${config.androidversion},预更新版本：${detail.oldversion} 线上版本：${detail.newversion}`;
				console.log(text);
				// uni.showToast({
				// 	title:text,
				// 	icon:'none'
				// })
				console.log(config.androidversion == data.versionCode)
				console.log(config.androidversion == detail.oldversion)
				if ((config.androidversion == data.versionCode) || (config.androidversion == detail.oldversion)) {
					data = {}
				}
			} else {
				data = {
					versionCode: detail.ios_newversion,
					versionName: '更新',
					versionInfo: detail.content,
					downloadUrl: detail.ios_url,
					forceUpdate: detail.enforce
				}
				let text = `当前版本:${config.iosversion},预更新版本：${detail.ios_oldversion},最新版本：${detail.ios_newversion}`;
				console.log(text);
				if ((config.iosversion == detail.ios_newversion) || (config.iosversion == detail.ios_oldversion)) {
					data = {}
				}
			}
			console.log("准本更新版本信息：", data);
...
export default function(isPrompt = false) {
	getCurrentNo(version => {
		...
		getServerNo(version.version, isPrompt, res => {
			if (res.forceUpdate) {
