...
export default {
	...
	methods: {
		toSeconds() {
			let that = this;
			var nowtime = new Date().getTime()/1000, //获取当前时间
				endtime = new Date(that.timestamp).getTime() ; //定义结束时间
		...
		countDown() {
			let that = this; 
			var seconds = that.toSeconds();
			let day, hour, minute, second;
			if (seconds > 0) {
				day = Math.floor(seconds / (60 * 60 * 24)); //计算天数 
				hour = Math.floor((seconds%(60*60*24))/(60*60)); //计算小时数
				minute = Math.floor((seconds%(60*60*24))%(60*60)/60); //计算分钟数 
				second = Math.floor((seconds%(60*60*24))%(60*60)%60); //计算秒数
			} else {
				this.timeUp();
			}
			if (day < 10) {
...
@import '~@/uni.scss';
$countdown-height: 45rpx;
$countdown-width: 45rpx;
