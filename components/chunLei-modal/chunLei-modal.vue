	...
	<view class="mask" @tap="tapMask" :style="{ backgroundColor: value ? `rgba(0,0,0,${opacity})` : 'rgba(0,0,0,0)' }" :class="!value ? '' : 'mask-show'" @touchmove.stop.prevent>
		<block v-if="type == 'default'">
			<view class="default-view" @tap.stop>
				...
				<view class="content word-break" v-if="mData.title" style="padding-top: 20upx; padding-bottom: 40upx">{{ mData.content }}</view>
				<view class="content word-break" v-else>{{ mData.content }}</view>
				<view class="modelBtn">
					...
					<view class="confirm" @tap="tapConfirm" :style="{ color: mData.confirmColor ? mData.confirmColor : '' }">
						{{ mData.confirmText ? mData.confirmText : '确认' }}
					</view>
				</view>
			</view>
		</block>
			...
			<view class="select-view" @tap.stop>
				<checkbox-group @change="checkboxChange">
					<view v-for="(item, index) in mData" :key="index" class="select-box">
						...
						<view class="select-content"><checkbox :value="item.title" :color="item.radioColor ? item.radioColor : '#001AFF'" :checked="item.flag"></checkbox></view>
					</view>
				</checkbox-group>
			</view>
		</block>
		<block v-if="type == 'advert'">
			<view class="advert-view">
				<image
					...
					class="image confirm"
					@tap.stop="tapConfirm"
					:style="{ width: mData.width ? mData.width : '500rpx', height: mData.height ? mData.height : '700rpx' }"
				></image>
		...
		<block v-if="type == 'notify'">
			<view class="notify-view">
				...
				<view class="content word-break" v-if="mData.title" style="padding-top: 20upx; padding-bottom: 40upx">{{ mData.content }}</view>
				<view class="content word-break" v-else style="padding: 80upx;">{{ mData.content }}</view>
		...
		<block v-if="type == 'input' && value">
			<view class="input-view" @tap.stop>
				...
				<view class="modelBtn">
					<view class="cancel" @tap="tapCancel" :style="{ color: mData.cancelColor ? mData.cancelColor : '' }">{{ mData.cancelText ? mData.cancelText : '取消' }}</view>
					<view class="confirm" @tap="inputConfirm" :style="{ color: mData.confirmColor ? mData.confirmColor : '' }">
						{{ mData.confirmText ? mData.confirmText : '确认' }}
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		showModal(option) {
			this.reject = '';
			this.resolve = '';
			(this.type = option.type ? option.type : 'default'), (this.mData = option.mData ? option.mData : {}), (this.value = true);
			//#ifdef APP-PLUS
			this.tabMask.show();
			//#endif
