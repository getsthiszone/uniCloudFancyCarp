<template>
	<view>
		<view v-if="loading" class="skeleton" :class="{ animate: animate }" :style="{ justifyContent: flexType }">
			<!-- 轮播图 -->
			<!-- v-if="imgTitle" -->
			<view v-if="showtype=='imgTitle'" class="skeleton-imgTitle" style="width: 95%;border-radius: 10px;height: 100px;display: block;"></view>
			<!-- 头像图 -->
			<!-- v-if="showAvatar && !imgTitle" -->
			<view
				v-if="showtype=='showAvatar'"
			...
			></view>
			<!-- 文字条 -->
			<!-- v-if="showTitle && !imgTitle" -->
			<view class="skeleton-content" v-if="showtype=='showTitle'"> 
				<view class="skeleton-title" :style="{ width: titleWidth }"></view>
				<view class="skeleton-rows">
					<view class="skeleton-row-item" v-for="(item, index) in rowList" :key="index" :style="{ width: item.width }"></view></view>
			</view>
			<!-- v-if="zview" -->
			<view class="skeleton-content" v-if="showtype=='zview'"> 
				<view class="skeleton-title" :style="{ width: zviewW, height: zviewH }"></view>
			</view>
		</view>
